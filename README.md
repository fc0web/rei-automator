...
Rei Automatorは、Rei言語をベースとした直感的なPC自動操作ツールです。日本語の自然言語入力からReiコードを生成し、ユーザーが確認した上で実行する三層構造を採用しています。

### 主な特徴

- 📷 **簡単キャプチャ**: 画面領域をドラッグで選択、自動座標取得
- 🎯 **座標指定**: クリック先をマウスで指定、座標自動記録
- 🔍 **画像認識**: テンプレートマッチングによるアプリ横断の自動化
...
- 📖 [取扱説明書（日本語）](docs/MANUAL_JA.md)
- 📖 [User Manual (English)](docs/MANUAL_EN.md)

## ダウンロード

[GitHub Releases](https://github.com/fc0web/rei-automator/releases/tag/v0.4.0) から入手できます。

- **Portable版** (ReiAutomator-0.4.0-portable.exe) — インストール不要、そのまま実行
- **インストーラー版** (Rei Automator Setup 0.4.0.exe) — デスクトップショートカット・スタートメニュー登録付き
...
詳細は [取扱説明書](docs/MANUAL_JA.md) をご覧ください。
